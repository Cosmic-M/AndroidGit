...
import com.example.cosmic_m.footballFakts.database.FootballBaseHelper;
...
import com.example.cosmic_m.footballFakts.database.SchemaDB.EventTable;
...
public class SingletonLeague {
    ...
    //private Map<String, TeamStanding> mTeamsStanding;//key=mLeagueCaption
    ...
    public void updateAndInsertTeamStanding(TeamStanding ts){
        ...
        Log.i(TAG, "LEAGUE_CAPTION = " + leagueCaption);
    ...
    public void updateAndInsertEvents(Map<String, Event> eventsMap) {
        ...
        try {
            if (cursor.getCount() == 0) {
                ...
                long end = System.currentTimeMillis();
                Log.i(TAG, "update " + allKeys.length + " rows for " + (end - start) + " ms");
            }
        }
        finally {
            cursor.close();
...
//            }
//            else {
//                if (eventsMap.get(allKeys[i]).getStatus() == null){
//                    Log.i(TAG, "updateAndInsertEvent : continue / without updates or inserts");
//                    continue;
//                }
//                else{
////                else if (eventsMap.get(allKeys[i]).getStatus().equals("FINISHED")){
////                    cursor = mDataBase.query(EventTable.NAME, null, EventTable.Cols.STATUS + " !=? ", new String[]{"FINISHED"}, null, null, null);
//                    if (cursor.getCount() != 0) {
//                        mDataBase.update(EventTable.NAME, getContentValuesOfEvent(eventsMap.get(allKeys[i])),
//                                EventTable.Cols.MATCH_ID + " =? ", new String[]{allKeys[i]});
//                        Log.i(TAG, "update element in " + EventTable.NAME);
//                    }
//                    cursor.close();
//               }
//            }
